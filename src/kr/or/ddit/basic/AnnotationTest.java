...
public class AnnotationTest {
	public static void main(String[] args) {
		...
		for(Method m : declaredMethods) {
			...
			for(int i = 0; i < m.getDeclaredAnnotation(PrintAnnotation.class).count(); i++) { // PrintAnnotation의 count값 만큼...
...
1. 리플렉션은 클래스 또는 멤버변수, 메서드, 생성자에 대한 정보를 가져오거나 수정할수 있다.
...
6. Reflection API의 기능은 뛰어나지만, 약간의 오버헤드가 발생한다.
	(느린수행속도, 보안취약성, 권한 문제등) 그러므로, 가급적 마지막 수단으로 사용하도록 고려되어야 한다.
 */
	}
}
